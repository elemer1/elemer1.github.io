...
**Proposition 3 (no seed, no progress).** *In $\mathcal{M}_0$, the ignorance formula $\theta$ is true at every world. Announcing it therefore removes nothing: $\mathcal{M}_0|_{\theta} = \mathcal{M}_0$, and the same holds for every iterate. Without the father's announcement, the children can broadcast their ignorance forever and the model never moves.* (Machine check V4.)
