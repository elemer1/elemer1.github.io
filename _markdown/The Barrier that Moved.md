...
Because $T$ preserves $\mu$, the operator $U$ preserves the $L^2$ norm — $\int |f\circ T|^2 d\mu = \int |f|^2 d\mu$ — so $U$ is a linear **isometry** on the Hilbert space $L^2(\Omega,\mu)$ (a unitary operator when $T$ is invertible). The time average is now an average of operators, $\frac1N\sum_{n<N} U^n f$, and ergodicity, by Result 1, is the statement that the only vectors fixed by $U$ are the constants. This recasting turns a question about chaotic trajectories into a question about a tame linear operator on a Hilbert space, and in that setting John von Neumann proved the first ergodic theorem.
...
> **Result 10 (Malthusian ergodicity).** The mean-reverting income process above with $|\rho| < 1$ is **stationary and ergodic**, with a stationary distribution of mean $s$ and variance $\dfrac{\sigma_\varepsilon^2}{1 - \rho^2}$. Consequently the time average of income along any single history and the ensemble average across the population both converge to the same value:
...
> **Maximal Ergodic Theorem.** For $f \in L^1(\mu)$, let $A = \big\{\, \omega : \sup_{n \ge 1} S_n f(\omega) > 0 \,\big\}$. Then $\displaystyle\int_{A} f \, d\mu \ge 0.$
...
Taking the maximum over $1 \le k \le N$ gives $\max_{1 \le k \le N} S_k f \le f + (M_N)\circ T$. Now restrict to the set $A_N = \{M_N > 0\}$. On $A_N$ the maximum defining $M_N$ exceeds $S_0 f = 0$, so it is attained at some $k \ge 1$, whence $M_N = \max_{1 \le k \le N} S_k f$ there. Therefore on $A_N$,
...
Finally $A_N \uparrow A$ as $N \to \infty$, and $|f| \in L^1$, so dominated convergence gives $\int_A f\,d\mu = \lim_N \int_{A_N} f\,d\mu \ge 0$. $\qquad\blacksquare$
...
Since $\alpha < \beta$, this forces $\mu(E) = 0$. Taking the union over all rational pairs $\alpha < \beta$, the set $\{\underline f < \overline f\}$ has measure zero, so $A_n f$ converges almost everywhere to a $T$-invariant limit $f^* := \overline f = \underline f$.

### A.3 Identifying the limit; $L^1$ convergence

It remains to show $f^* \in L^1$ with $\int f^* d\mu = \int f\,d\mu$, and the ergodic specialization. First suppose $f \ge 0$. By Fatou's lemma, $\int f^*\,d\mu \le \liminf_n \int A_n f\,d\mu = \int f\,d\mu < \infty$ (the last equality because $\int A_n f\,d\mu = \int f\,d\mu$ for every $n$, as $T$ preserves $\mu$), so $f^* \in L^1$. The reverse inequality $\int f^* \ge \int f$, giving equality, follows from the maximal theorem applied to the sets $\{f^* < \beta\}$ for $\beta < \int f$ (a standard truncation argument; see Walters, *An Introduction to Ergodic Theory*, Thm. 1.14). For general $f \in L^1$, decompose $f = f^+ - f^-$. The $L^1$ convergence $A_n f \to f^*$ then follows because $\{A_n f\}$ is uniformly integrable (the averages of an $L^1$ function are uniformly integrable) and converges a.e., and a.e. convergence plus uniform integrability gives $L^1$ convergence.

Finally, the limit $f^*$ is $T$-invariant, so it is measurable with respect to the invariant $\sigma$-algebra $\mathcal I = \{A : T^{-1}A = A\}$, and the identity $\int_E f^*\,d\mu = \int_E f\,d\mu$ for all invariant $E$ (same maximal-theorem argument applied on $E$) identifies it as the conditional expectation $f^* = \mathbb E[f \mid \mathcal I]$.
...
> **Mean Ergodic Theorem.** Let $U$ be a unitary operator on a Hilbert space $H$, let $V = \{x \in H : Ux = x\}$ be its subspace of invariant vectors, and let $P$ be the orthogonal projection onto $V$. Then for every $x \in H$,
...
using $\|Ux\| = \|x\|$. Thus $B^\perp = V$, and taking orthogonal complements, $\overline B = V^\perp$. Every $x \in H$ decomposes uniquely as $x = v + w$ with $v = Px \in V$ and $w \in \overline B$.
...
The averaging operators are uniform contractions, $\|A_N\| \le \frac1N\sum_{n=0}^{N-1}\|U^n\| = 1$, so convergence to $0$ extends from the dense subspace $B$ to its closure $\overline B$: for $w \in \overline B$ and $\varepsilon > 0$, pick $w' \in B$ with $\|w - w'\| < \varepsilon$; then $\|A_N w\| \le \|A_N(w - w')\| + \|A_N w'\| \le \varepsilon + \|A_N w'\|$, and the last term $\to 0$.
...
so $|W_t| = O\!\big(\sqrt{t\ln\ln t}\,\big) = o(t)$ and thus $W_t/t \to 0$ a.s. (An elementary alternative: along integers, $W_n/n = \frac1n\sum_{k=1}^n (W_k - W_{k-1})$ is an average of i.i.d. standard normals, which $\to 0$ a.s. by the strong law; the fluctuation of $W$ between consecutive integers is a.s. $o(n)$ by the Brownian modulus of continuity, so the limit holds along all real $t$.) Therefore
...
> **Theorem (Gnedenko–Kolmogorov; Lévy).** Let $X_1, X_2, \dots$ be i.i.d. with regularly varying tails of index $\alpha \in (0,2)$ — that is, $\mathbb P(|X| > x) = x^{-\alpha} L(x)$ for a slowly varying $L$. Then there exist norming constants $a_n = n^{1/\alpha}\tilde L(n)$ (with $\tilde L$ slowly varying) and centering $b_n$ such that
...
Let $B_t = mt + \sigma W_t$ with $B_0 = 0$, drift $m > 0$, volatility $\sigma > 0$. Fix a level $a > 0$ and let $\tau = \inf\{t \ge 0 : B_t = -a\}$ be the first time the process falls to $-a$. We compute $\mathbb P(\tau < \infty)$.

**An exponential martingale.** For any $\lambda$, $\mathbb E[e^{\lambda B_t}] = \exp\!\big(t(\lambda m + \tfrac12\lambda^2\sigma^2)\big)$. Choosing the nonzero root of $\lambda m + \tfrac12\lambda^2\sigma^2 = 0$, namely

$$\lambda = -\frac{2m}{\sigma^2} < 0,$$

makes $Z_t := e^{\lambda B_t}$ a martingale with $\mathbb E[Z_t] = 1$.

**Optional stopping.** Apply the optional stopping theorem at $\tau \wedge t$: $\mathbb E[Z_{\tau \wedge t}] = 1$. We control the limit $t \to \infty$. For $t < \tau$ we have $B_t > -a$, and since $\lambda < 0$ this gives $Z_t = e^{\lambda B_t} < e^{-\lambda a} = e^{2ma/\sigma^2}$, so $Z_{\tau\wedge t}$ is bounded. On $\{\tau < \infty\}$, $Z_{\tau} = e^{\lambda(-a)} = e^{2ma/\sigma^2}$. On $\{\tau = \infty\}$, the positive drift $m > 0$ forces $B_t \to +\infty$ almost surely, so $Z_t = e^{\lambda B_t} \to 0$. Bounded convergence yields
...
**The inversion.** The global optimum is $f^\star = \arg\max\{\,g_K(f_c)\ (\text{if }f_c \le 1-\kappa),\ g_{\text{floor}}(1)\,\}$. Two limits make the mechanism transparent:
...
*Bet-hedging (corrected):* safe genotype $A$: arithmetic $=$ geometric $= 1.30$; bold genotype $B = \{2.5, 0.5\}$: arithmetic $= 1.50$, geometric $= 1.118$. The bold genotype has higher expected (ensemble) fecundity yet lower geometric (time-average) growth, and is driven extinct by the safe one — Result 11.
